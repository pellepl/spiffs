  ...
  while (res == SPIFFS_OK && gc.state != FINISHED) {
    ...
    case FIND_OBJ_DATA:
      if (gc.obj_id_found) {
        ...
        SPIFFS_CHECK_RES(res); // TODO on borked systems we might get an ERR_NOT_FOUND here
        // this might be handled by simply deleting the page as it is not referenced
        // from anywhere
